...
parser.add_argument('--channels1', type=int, default=256)
parser.add_argument('--channels2', type=int, default=128)
parser.add_argument('--batch', type=int, default=2900)
...
# full_data.edge_weight = 1.0 / degree(col, full_data.num_nodes)[col]
...
class GNN(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.conv1 = pyg_nn.GraphConv(in_channels, out_channels)
        self.conv2 = pyg_nn.GraphConv(out_channels, out_channels)
...
############################################################################################
        ...
        enc = self.excelformer_encoder.encoder_dict[stype.numerical.value]
        self.register_buffer('mean', enc.mean)
        self.register_buffer('std',  enc.std)
...
############################################################################################
        ...
        self.excelformer_decoder = ExcelFormerDecoder(in_channels,
                                                    hidden_channels, num_cols)
        
        self.GNNConv = GNN(num_cols, hidden_channels)
    ...
    def forward(
        ...
        for excelformer_conv in self.excelformer_convs:
            x1 = excelformer_conv(x1)
        x1 = self.excelformer_decoder(x1)
        
        x2 = self.GNNConv(x=feat, edge_index=data.edge_index, edge_weight=data.edge_weight)
...
model = ExcelFormer(
    in_channels=args.channels1,
    hidden_channels=args.channels2,
...
print(f'The PARALLEL_state -> Task Type: {args.task_type}; Idx: {args.idx}; Seed: {args.seed}')
